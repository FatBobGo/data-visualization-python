...
def generate_plotly_config(df: pd.DataFrame, rec: ChartRecommendation, palette: str = "default") -> dict[str, Any]:
    ...
    Supports multi-Y: if rec.columns contains multiple Y columns (excluding x_column),
    generates one trace per Y column for LINE, SCATTER, AREA, and BAR charts.
    """
    ...
    # Determine Y columns: multi-Y from columns list (excluding x_col), fallback to single y_col
    y_cols = [c for c in rec.columns if c != x_col] if rec.columns else []
    ...
    if ct == ChartType.BAR:
        if is_multi_y:
            ...
            for i, yc in enumerate(y_cols):
                if yc in df.columns:
                    data.append({"type": "bar", "name": yc, "x": df[x_col].tolist(),
                                  "y": df[yc].tolist(), "marker": {"color": colors[i % len(colors)], "cornerradius": 4}})
        ...
        if not is_multi_y and y_col:
            layout["yaxis"]["title"] = {"text": y_col}

    elif ct == ChartType.LINE:
        for i, yc in enumerate(y_cols):
            if yc in df.columns:
                data.append({"type": "scatter", "mode": "lines+markers", "name": yc,
                              "x": df[x_col].tolist(), "y": df[yc].tolist(),
        ...
        for i, yc in enumerate(y_cols):
            if yc in df.columns:
                data.append({"type": "scatter", "mode": "markers", "name": yc,
                              "x": df[x_col].tolist(), "y": df[yc].tolist(),
                              "marker": {"color": colors[i % len(colors)], "size": 8, "opacity": 0.7,
                              "line": {"width": 1, "color": colors[(i + 1) % len(colors)]}}})
        layout["xaxis"]["title"] = {"text": x_col}
        if not is_multi_y and y_col:
            layout["yaxis"]["title"] = {"text": y_col}
        if is_multi_y:
        ...
        fill_opacities = [0.15, 0.10, 0.08, 0.06, 0.05]
        for i, yc in enumerate(y_cols):
            if yc in df.columns:
                opacity = fill_opacities[i % len(fill_opacities)]
                c = colors[i % len(colors)]
                # Convert hex to rgba for fill
                r, g, b = int(c[1:3], 16), int(c[3:5], 16), int(c[5:7], 16)
                data.append({"type": "scatter", "mode": "lines", "name": yc,
                              "x": df[x_col].tolist(), "y": df[yc].tolist(),
                              "fill": "tozeroy", "fillcolor": f"rgba({r},{g},{b},{opacity})",
...
def build_chart_from_request(df: pd.DataFrame, request: ChartRequest) -> ChartResponse:
    ...
    Supports multi-Y: if request.columns contains Y column names, they are used
    as multiple series for line/scatter/area/bar charts.
    """
    ...
    # Aggregation only for single-Y charts
    if request.aggregation and request.x_column and request.y_column and len(request.columns) <= 1:
